...
from utils.loss import DiceCoefMultilabelLoss, SurfaceLoss
...
def predict_model(dataset, model, device ='cpu', save_images= True, comments = ""):
    ...
    with torch.no_grad():
        for d, data in enumerate(dataloader):
            ...
            dice_accuracy = dice_coefficient_multiclass(output_masks, mask,  numLabels=6).item()
            print("Image [{0}]:  \t Dice score {1:3f}".format(d, dice_accuracy))
            dice_metric[d] =  dice_accuracy
            if save_images:
                save_output_image(image, output_masks, mask, d, comments)
    print("Test mean acuraccy: \t {0:3f}]:".format(np.mean(dice_metric)) )
    return dice_metric

def save_output_image(image, output, mask, id, comments = ""):
    ...
    # plt.show(block=True)
    plt.imsave('./results/'+comments+'/pred_{}.png'.format(str(id).zfill(3)), output_image)
    plt.imsave('./results/'+comments+'/gt_{}.png'.format(str(id).zfill(3)),  mask_image)
...
if __name__ == '__main__':
    ...
    if args.pretrained:
        model.load_state_dict(torch.load('./weights/{}/unet_model_checkpoint.pt'.format(model_params)))

    dice_accuracy = predict_model(dataset, model, device='cpu', save_images= True, comments = model_params)
