...
from utils.loss import DiceCoefMultilabelLoss, LossMeter, DiceSurfaceLoss, DiceLoss
from model.dilated_unet import Segmentation_model, Branched_model
...
class TrainerBranched:
    def __init__(self, model,
        ...
        self.loss_bin = DiceLoss(n_classes=2)
    ...
    def train_epoch(self, epoch):
        ...
        dice_edema =  LossMeter()
        self.net.train()
        # pid = os.getpid()
        for iter, data in enumerate(self.train_dataloader):
            image , mask=  torch.squeeze(data['image'],dim=0).to(self.device),  torch.squeeze(data['scar'],dim=0).to(self.device)
            mask_bin = torch.squeeze(data['edema'], dim=0).to(self.device)
            self.optim.zero_grad()
            output_bin, output = self.net(image)
            output_scar = nn.Softmax2d()(output)
            output_edema = nn.Softmax2d()(output_bin)
            ...
            dice_metric.update(l_scar, output.size(0))
            output_mask_edema = one_hot_mask(output_edema, channel_axis=1)
            l_edema = dice_coefficient_multiclass(output_mask_edema, mask_bin).item()
            dice_edema.update(l_edema, output.size(0))

            if iter % self.logs == 0: # Print logs
                print('Epoch [{0}][{1}/{2}]:\t' 'Loss {loss:.4f} '.format(epoch, iter, len(self.train_dataloader), loss=loss.item() ))
            del image, mask, output,l
    ...
    def validation(self):
        ...
        with torch.no_grad():
            ...
            del image, mask, output, output_scar, l
            torch.cuda.empty_cache()
        val_loss = loss_meter.get_avg_loss()
        self.loss_logs['val_loss'].append( loss_meter.get_avg_loss())
        self.loss_logs['val_dice'].append( dice_metric.get_avg_loss()+dice_edema.get_avg_loss())
        print('Validation: \t Mean Loss:  {0:.5f}   \t    Dice scar:  {1:.5f}   \t    Dice scar+edema:  {2:.5f}   \n'.format( val_loss , dice_metric.get_avg_loss(),dice_edema.get_avg_loss() ))
...
if __name__ == '__main__':
    ...
    parser.add_argument("-bs", "--batch_size", help="batch size of training", type=int, default=1)
    ...
    assert args.batch_size==1, "The batçh size for patch dataloader must be 0"
    ...
    for i in range(CV):
        ...
        model = Branched_model(filters=args.n_filter,
                                        in_channels=3,
        ...
        train_obj = TrainerBranched(model,
