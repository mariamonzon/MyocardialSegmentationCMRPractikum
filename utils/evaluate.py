        ...
        with tf.Session() as sess:
            ...
            for idx_file, fid in enumerate(test_list):
                ...
                for ii in range(int(np.floor(data.shape[2] // self.batch_size))):
                    ...
                    if TEST_MODALITY=='CT':
                        data_batch = np.subtract(np.multiply(np.divide(np.subtract(data_batch, -2.8),
                                                                       np.subtract(3.2, -2.8)), 2.0),1) # {-2.8, 3.2} need to be changed according to the data statistics
                    elif TEST_MODALITY=='MR':
                        data_batch = np.subtract(np.multiply(np.divide(np.subtract(data_batch, -1.8),
                                                                       np.subtract(4.4, -1.8)), 2.0),1)  # {-1.8, 4.4} need to be changed according to the data statistics
