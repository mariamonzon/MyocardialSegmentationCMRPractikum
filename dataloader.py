...
class ImageProcessor:
    ...
    def split_data(img_path):
        ...
        df_train = pd.read_csv(img_path, delimiter=';')
        print(df_train.keys())
...
class DataGenerator:
    ...
    def get_image_paths(self, id):
        if self._phase == "train":
            img_path = './input/train/{}'.format(id)
            mask_path = './input/masks/{}'.format(id)
        else:
            img_path = './input/train/{}'.format(id)
            mask_path = './input/masks/{}'.format(id)
...
if __name__ == "__main__":
    ...
    ids_valid = ImageProcessor.split_data("./input/images_masks_full.csv")
    bs = 16
    num_samples = 1000
